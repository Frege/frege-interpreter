...
public class JavaUtils {
  ...
  public static TEither execute(final String javaSource, final String className,
    ...
    try {
      ...
      return TEither.DLeft.mk(Delayed.delayed(e.getMessage()));
    }
  }
  ...
  private static List<Lazy> toJavaListLoop(final TList list, final List<Lazy> acc) {
    if (list._constructor() == 0) { //Nil
      return acc;
    } else {
      final DCons cons = list._Cons();
      final Lazy elem = ((Lazy) cons.mem1).forced();
      acc.add(elem);
      return toJavaListLoop((TList)cons.mem2.forced(), acc);
    }
  }
  
  public static List<Lazy> toJavaList(final TList list) {
    final List<Lazy> jlist = toJavaListLoop(list, new ArrayList<Lazy>());
